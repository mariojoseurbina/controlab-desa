...
$ScriptPath = "$InstallDir\local_sniff_tshark.ps1"
...
$SourceScript = Join-Path $PSScriptRoot "local_sniff_tshark.ps1"
if (Test-Path $SourceScript) {
    Copy-Item -Path $SourceScript -Destination $ScriptPath -Force
    Write-Host "[✔] Copiado capturador a: $ScriptPath"
} else {
    Write-Host "[❌ ERROR] No se encontró el archivo local_sniff_tshark.ps1 en el origen."
    exit
}
